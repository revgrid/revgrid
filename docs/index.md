...
---------|-------------|---------
 0.4.0   | ?? Aug 2023 | Fix Paint AutoWidening<br>Add StandardTextPainter (contains behavior previously in StandardTextCellPainter)<br>Remove StandardTextCellPainter<br>Add StandardCheckboxPainter (contains behavior previously in StandardCheckboxCellPainter)<br>Remove Text settings (move behavior into Standard settings)<br>Rename RevRecordMainDataServer to RevRecordDataServer<br>Split out "Multi Heading" and "Single Heading" server sets from "Data Row Array" server set<br>Delete RevRecordHeaderDataServer (use Single Heading server instead)<br>Rename simpletestapp to dratestapp (short for datarowarraytestapp)
