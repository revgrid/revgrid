...
export class RowSelection extends Feature {
    ...
    override handleMouseDown(grid: Hypergrid, event: CellEvent) {
        ...
        const rowNumberClick = leftClick &&
        ...
        const rowSelectable = grid.properties.rowSelection && (leftClick && grid.properties.autoSelectRows || rowNumberClick);
    ...
    override handleKeyDown(grid: Hypergrid, event: Canvas.KeyboardSyntheticEvent) {
        let handler: (this: void, grid: Hypergrid, detail: Canvas.SyntheticEventDetail.Keyboard) => void;
        if (
    ...
    handleMouseDragCellSelection(grid: Hypergrid, y: number, keys: string[]) {
     ...
     */
    checkDragScroll(grid: Hypergrid, mousePoint: Point) {
        if (
     ...
     * @param keys - array of the keys that are currently pressed down
     */
    extendSelection(grid: Hypergrid, y: number, keys: string[]) {
     ...
     */
    moveShiftSelect(grid: Hypergrid, offsetY: number) {
     ...
     * @param offsetY - y coordinate to start at
     */
    override moveSingleSelect(grid: Hypergrid, offsetY: number, shift?: boolean) {
...
function moveCellSelection(grid: Hypergrid) {
    let rows: number[];
