...
export class RevCellClickUiController<BGS extends RevBehavioredGridSettings, BCS extends RevBehavioredColumnSettings, SF extends RevSchemaField> extends RevUiController<BGS, BCS, SF> {
     ...
     *
    //  * @remarks The URL is found in the cell's link property, which serves two functions:
    ...
    //  * 4. **Open the URL.** The link is then opened by Hypergrid#windowOpen|grid.windowOpen. If `link` is an array, it is "applied" to `grid.windowOpen` in its entirety; otherwise, `grid.windowOpen` is called with the link as the first argument and linkTarget as the second.
