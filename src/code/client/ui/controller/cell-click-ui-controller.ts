...
export class RevCellClickUiController<BGS extends RevBehavioredGridSettings, BCS extends RevBehavioredColumnSettings, SF extends RevSchemaField> extends RevUiController<BGS, BCS, SF> {
    ...
    /**
     * Open the cell's URL. NOT WORKING.
     *
    //  * @remarks The URL is found in the cell's {link link} property, which serves two functions:
    //  * 1. **Renders as a link.** When truthy causes {link SimpleCell} cell renderer to render the cell underlined with {link linkColor}. (See also {link module:defaults.linkOnHover|linkOnHover} and {link module:defaults.linkColorOnHover|linkColorOnHover}.) Therefore, setting this property to `true` will render as a link, although clicking on it will have no effect. This is useful if you wish to handle the click yourself by attaching a `'rev-click'` listener to your hypergrid.
