...
import { RectangleInterface } from '../lib/rectangular';
...
export abstract class CellEditor {
    ...
    constructor(public readonly grid: Hypegrid, options?: CellEvent, template?: string) {
    ...
    keyup(e: KeyboardEvent) {
        ...
        if (GridProperties.mappedNavKey(cellProps.gridProperties, keyChar, e.ctrlKey)) {
            if (
                !specialKeyup &&
...
export namespace CellEditor {
    export type Constructor = new (grid: Hypegrid, options?: CellEvent, template?: string) => CellEditor;
