...
interface SubrowSnapshot extends RenderCell.SubrowSnapshot {
    value: string;
...
function renderSingleLineText(gc: CanvasRenderingContext2DEx, config: CellPaintConfig, val: string, leftPadding: number, rightPadding: number) {
    ...
    if (config.columnAutosizing) {
        const truncatedResult = gc.getTextWidthTruncated(val, truncateWidth, config.truncateText, false, rightHaligned);
        ...
        const truncatedResult = gc.getTextWidthTruncated(val, truncateWidth, config.truncateText, true, rightHaligned);
        minWidth = 0;
