...
export class RevSimpleAdapterSet {
    private _schemaAdapter = new RevSimpleSchemaAdapter();
    private _mainAdapter = new RevSimpleMainAdapter();
    private _headerAdapter = new RevSimpleHeaderAdapter();
    ...
    setData(data: RevSimpleAdapterSet.DataRow[] | (() => RevSimpleAdapterSet.DataRow[]), headerRowCount = -1) {
        if (data === undefined) {
            ...
            if (!Array.isArray(dataRows)) {
                ...
                let schema: RevSimpleSchemaAdapter.Column[];
                if (headerRowCount < 0) {
    ...
    private extractSchemaAndHeadersFromData(
        ...
        if (headerCount === 0) {
            ...
            const schema = new Array<RevSimpleSchemaAdapter.Column>(columnCount);
    ...
    private calculateSchemaFromData(dataRows: RevSimpleAdapterSet.DataRow[], keyIsHeader: boolean): RevSimpleSchemaAdapter.Column[] {
...
interface ExtractSchemaAndHeadersFromDataResult {
    schema: RevSimpleSchemaAdapter.Column[];
