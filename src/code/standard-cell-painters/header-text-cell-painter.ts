...
export class HeaderTextCellPainter extends TextCellPainter {
    ...
    override paint(prefillColor: string | undefined): number | undefined {
        ...
        const textFont = selection.isColumnSelected(activeColumnIndex) ? settings.foregroundSelectionFont : settings.font;

        const textColor = gc.cache.strokeStyle = isSelected
            ? settings.foregroundSelectionColor
            : settings.color;

        const backgroundColor = this._settingsAccessor.backgroundColor;
