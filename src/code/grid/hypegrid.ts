...
/** @public */
export class Hypegrid implements SelectionDetail {
    options: Hypegrid.Options;
    ...
    // Begin Events mixin
    eventlistenerInfos = new Map<keyof Canvas.EventMap | keyof Hypegrid.EventMap, Hypegrid.ListenerInfo[]>();
    // End Events mixin
 ...
 *
 */
    constructor(options?: Hypegrid.Options);
    constructor(container: string | HTMLElement, options?: Hypegrid.Options);
    constructor(containerOrOptions: string | HTMLElement | Hypegrid.Options, options?: Hypegrid.Options) {
    ...
    terminate() {
        ...
        Hypegrid.grids.splice(Hypegrid.grids.indexOf(this), 1);
     ...
     */
    renderOverridesCache = Hypegrid.RenderOverridesCache.empty; // : {};
    ...
    reset(options?: Hypegrid.Options) {
        ...
        this.renderOverridesCache = Hypegrid.RenderOverridesCache.empty; //{};
        this.clearMouseDown();
    ...
    setFormatter(options?: Hypegrid.LocalizationOptions) {
        options = options ?? {};
        this.localization = new Localization(
            options.locale || Hypegrid.defaultLocalizationOptions.locale,
            options.numberOptions || Hypegrid.defaultLocalizationOptions.numberOptions,
            options.dateOptions || Hypegrid.defaultLocalizationOptions.dateOptions
        );
    }
    ...
    getValue(x: number, y: number, subgrid?: Subgrid) {
        const column = this.behavior.getActiveColumn(x);
     ...
     */
    setData(data: Hypegrid.Options.Data, options?: Hypegrid.Options) {
        // if (!this.behavior) {
     ...
     * @private
     */
    initCanvas(options?: Hypegrid.Options) {
        if (!this.canvasDiv) {
    ...
    getAllColumns(begin?: number, end?: number): Column[] {
        const columns = this.behavior.getAllColumns();
    ...
    getAllColumnsWidth() {
        ...
        const activeColumnCount = behavior.getActiveColumnCount();
        const fixedColumnCount = behavior.getFixedColumnCount();

        let width = 0;
        for (let i = leftMostColIndex; i < activeColumnCount; i++) {
            width += behavior.getColumnWidth(i);

            if (i > leftMostColIndex) {
    ...
    calculateScrollerContentSize(viewportSize: number, gridRightAligned: boolean) {
        let result = this.getNonFixedColumnsWidth();
        if (result > viewportSize && !this.properties.scrollHorizontallySmoothly) {
            const columnCount = this.getColumnCount();
            const fixedColumnCount = this.getFixedColumnCount();
            if (columnCount > fixedColumnCount) {
                ...
                if (gridRightAligned) {
                    ...
                    let prevColumnGridLineFinish = this.getFixedColumnsWidth() - 1;
                    const lowestViewportFinish = prevColumnGridLineFinish + viewportSize;
                    let lowestViewportStartColumnIndex = fixedColumnCount;
                    let lowestViewportStartColumnFinish = prevColumnGridLineFinish + this.getColumnWidth(lowestViewportStartColumnIndex);
                    while (lowestViewportStartColumnFinish <= lowestViewportFinish) {
                        ...
                        lowestViewportStartColumnFinish = prevColumnGridLineFinish + (this.getColumnWidth(lowestViewportStartColumnIndex) + gridLinesVWidth);
                    }
                    result += (lowestViewportFinish - prevColumnGridLineFinish);
                } else {
                    ...
                    const highestViewportStart = result - viewportSize;
                    let nextColumnLeft = result;
                    let highestViewportStartColumnIndex = columnCount - 1;
                    let highestViewportStartColumnLeft = nextColumnLeft - this.getColumnWidth(highestViewportStartColumnIndex);
                    while (highestViewportStartColumnLeft > highestViewportStart) {
                        ...
                        highestViewportStartColumnLeft = nextColumnLeft - (this.getColumnWidth(highestViewportStartColumnIndex) + gridLinesVWidth);
                    }
                    result += (nextColumnLeft - highestViewportStart);
                }
            }
        }
    ...
    getColumnScrollableLeft(columnIndex: number) {
        const fixedColumnCount = this.getFixedColumnCount();
        if (columnIndex < fixedColumnCount) {
            ...
            const gridLinesVWidth = this.properties.gridLinesVWidth;
            let result = 0;
            for (let i = fixedColumnCount; i < columnIndex; i++) {
                result += this.getColumnWidth(i);
            }

            if (gridLinesVWidth > 0) {
                const scrollableColumnCount = columnIndex - fixedColumnCount;
                if (scrollableColumnCount > 1) {
                    result += (scrollableColumnCount - 1) * gridLinesVWidth;
                }
            }
     ...
     */
    setColumnWidth(columnOrIndex: number | Column, columnWidth: number) {
        if (this.abortEditing()) {
            return this.behavior.setColumnWidth(columnOrIndex, columnWidth);
        } else {
            return undefined;
        }
    }
    ...
    updateCursor() {
        ...
        if (
            ...
            const x = hoverGridCell.x + this.getHScrollViewportStartColumnIndex();
     ...
     * @desc Listeners added by this method should only be removed by {@link Hypegrid#removeEventListener|grid.removeEventListener} (or {@link Hypegrid#removeAllEventListeners|grid.removeAllEventListeners}).
     * @param eventName - The type of event we are interested in.
     * @param listener - The event handler.
     * @param internal - Used by {@link Hypegrid#addInternalEventListener|grid.addInternalEventListener} (see).
     */
    addEventListener<T extends keyof Canvas.EventMap>(eventName: T, listener: (e: Canvas.EventMap[T]) => void, internal?: boolean): void;
    addEventListener<T extends keyof Hypegrid.EventMap>(eventName: T, listener: (e: Hypegrid.EventMap[T]) => void, internal?: boolean): void;
    addEventListener(eventName: keyof Canvas.EventMap | keyof Hypegrid.EventMap, listener: EventListener, internal?: boolean): void {
        ...
        if (!alreadyAttached) {
            const info: Hypegrid.ListenerInfo = {
     ...
     * @desc The new listener is flagged as "internal." Internal listeners are removed as usual by {@link Hypegrid#removeEventListener|grid.removeEventListener}. However, they are ignored by {@link Hypegrid#removeAllEventListeners|grid.removeAllEventListeners()} (as called by {@link Hypegrid#reset|reset}). (But see {@link Hypegrid#removeAllEventListeners|grid.removeAllEventListeners(true)}.)
     *
     * Listeners added by this method should only be removed by {@link Hypegrid#removeEventListener|grid.removeEventListener} (or {@link Hypegrid#removeAllEventListeners|grid.removeAllEventListeners(true)}).
     * @param eventName - The type of event we are interested in.
     * @param listener - The event handler.
     */
     ...
     * @desc Removes the event listener with matching name and function that was added by {@link Hypegrid#addEventListener|grid.addEventListener}.
     *
     * NOTE: This method cannot remove event listeners added by other means.
     */

    removeEventListener<T extends keyof Canvas.EventMap>(eventName: T, listener: (e: Canvas.EventMap[T]) => void): void;
    removeEventListener<T extends keyof Hypegrid.EventMap>(eventName: T, listener: (e: Hypegrid.EventMap[T]) => void): void;
    removeEventListener<T extends (keyof Hypegrid.EventMap | keyof Canvas.EventMap)>(eventName: T, listener: EventListener): void;
    removeEventListener(eventName: keyof Canvas.EventMap | keyof Hypegrid.EventMap, listener: EventListener): void {
     ...
     * @desc Removes all event listeners added with {@link Hypegrid#addEventListener|grid.addEventListener} except those added as "internal."
            ...
            value.forEach(
                (info) => {
                    if (includeInternal || !info.internal) {
                        const eventName = key as (keyof Canvas.EventMap); // hack to workaround Typescript
                        this.removeEventListener(eventName, info.listener);
                    }
                }
            )
        }
    ...
    fireSyntheticColumnSortEvent(c: number, keys: string[]): boolean {
        const eventDetail: Hypegrid.ColumnSortEventDetail = {
            column: c,
            keys: keys
        }
        return dispatchGridEvent(this, 'fin-column-sort', false, eventDetail);
    }
    ...
    fireSyntheticEditorKeyUpEvent(inputControl: CellEditor, keyEvent: KeyboardEvent) {
        ...
        return dispatchGridEvent(this, 'fin-editor-keyup', false, eventDetail);
    }
    ...
    fireSyntheticEditorKeyDownEvent(inputControl: CellEditor, keyEvent: KeyboardEvent) {
        ...
        return dispatchGridEvent(this, 'fin-editor-keydown', false, eventDetail);
    }
    ...
    fireSyntheticEditorKeyPressEvent(inputControl: CellEditor, keyEvent: KeyboardEvent) {
        ...
        return dispatchGridEvent(this, 'fin-editor-keypress', false, eventDetail);
    }
    ...
    fireSyntheticEditorDataChangeEvent(inputControl: CellEditor, oldValue: unknown, newValue: unknown) {
        ...
        return dispatchGridEvent(this, 'fin-editor-data-change', true, eventDetail);
    }
    ...
    fireSyntheticRowSelectionChangedEvent() {
        const selectionDetail = new SelectionDetailAccessor(this);
        return dispatchGridEvent(this, 'fin-row-selection-changed', false, selectionDetail);
    }
    ...
    fireSyntheticColumnSelectionChangedEvent() {
        const selectionDetail = new SelectionDetailAccessor(this);
        return dispatchGridEvent(this, 'fin-column-selection-changed', false, selectionDetail);
    }
    ...
    fireSyntheticGridRenderedEvent() {
        const eventDetail: Hypegrid.GridEventDetail = {
            source: this,
        }
        return dispatchGridEvent(this, 'fin-grid-rendered', false, eventDetail);
    }
    ...
    fireSyntheticTickEvent() {
        ...
        return dispatchGridEvent(this, 'fin-tick', false, eventDetail);
    }
    ...
    fireSyntheticGridResizedEvent(e: Canvas.ResizeSyntheticEvent) {
        return dispatchGridEvent(this, 'fin-grid-resized', false, e.detail);
    }
    ...
    fireSyntheticTouchStartEvent(e: Canvas.TouchSyntheticEvent) {
        return dispatchGridEvent(this, 'fin-touchstart', false, e.detail);
    }
    ...
    fireSyntheticTouchMoveEvent(e: Canvas.TouchSyntheticEvent) {
        return dispatchGridEvent(this, 'fin-touchmove', false, e.detail);
    }
    ...
    fireSyntheticTouchEndEvent(e: Canvas.TouchSyntheticEvent) {
        return dispatchGridEvent(this, 'fin-touchend', false, e.detail);
    }
     ...
     * @returns Proceed; event was not [canceled](https://developer.mozilla.org/docs/Web/API/EventTarget/dispatchEvent#Return_Value `EventTarget.dispatchEvent`).
     */
    fireScrollEvent(eventName: 'fin-scroll-x' | 'fin-scroll-y', newValue: number, index: number, offset: number) {
        const eventDetail: Hypegrid.ScrollEventDetail = {
            value: newValue,
            index,
            offset,
        };
    ...
    fireRequestCellEdit(input: CellEditor, cellEvent: CellEvent, value: unknown) {
        ...
        return dispatchGridEvent(this, 'fin-request-cell-edit', true, eventDetail, cellEvent);
    }
    ...
    fireBeforeCellEdit(point: WritablePoint, oldValue: unknown, newValue: unknown, control: CellEditor) {
        ...
        return dispatchGridEvent(this, 'fin-before-cell-edit', true, eventDetail, point);
    }
    ...
    fireAfterCellEdit(point: WritablePoint, oldValue: unknown, newValue: unknown, control: CellEditor) {
        ...
        return dispatchGridEvent(this, 'fin-after-cell-edit', false, eventDetail, point);
    }
     ...
     *
     * All members are documented on the {@link Hypegrid} page.
    ...
    getColumnSelection() {
        ...
        const headerRowCount = this.getHeaderRowCount();
        const selectedColumnIndexes = this.getSelectedColumns();
        const result: Hypegrid.ColumnsDataValuesObject = {};
    ...
    getSelection(): Hypegrid.ColumnsDataValuesObject[] {
        const behavior = this.behavior;
        const subgrid = behavior.mainSubgrid;
        const selections = this.getSelections();
        const rects = new Array<Hypegrid.ColumnsDataValuesObject>(selections.length);
        ...
        selections.forEach(
            (selectionRect, i) => {
                ...
                const columns: Hypegrid.ColumnsDataValuesObject = {};
    ...
    getSelectedRow() {
        ...
        if (sels.length) {
            ...
            for (let c = 0; c < colCount; c++) {
                const column = behavior.getActiveColumn(c);
                row[column.header] = behavior.getValue(column.schemaColumn, c, topRow);
            }
    ...
    private getActiveAllOrSpecifiedColumns(hiddenColumns: boolean | number[] | string[]): Column[] {
        let columns: Column[];
        const allColumns = this.behavior.getAllColumns();
    ...
    scrollHBy(offsetX: number) {
        const max = this.sbHScroller.contentRange.finish;
        const oldValue = this.getHScrollViewportStartColumnIndex();
        const newValue = Math.min(max, Math.max(0, oldValue + offsetX));
        if (newValue !== oldValue) {
            this.handleHScrollerChange(newValue);
        }
    }
    ...
    scrollToMakeVisible(c: number, r: number) {
        ...
        if (c >= fixedColumnCount) {
            ...
            if ((delta = c - dw.origin.x) < 0) {
                const scrollableLeft = this.getColumnScrollableLeft(c);
                this.sbHScroller.scrollViewportStartTo(scrollableLeft);
                ...
                if ((c - dw.corner.x) > 0) {
                    this.sbHScroller.scrollViewportStartTo(this.renderer.getMinimumLeftPositionToShowColumn(c));
                }
            }
        }
    ...
    handleVScrollerChange(y: number) {
        ...
        if (y !== this.vScrollValue) {
            ...
            setTimeout(() => {
                ...
                this.fireScrollEvent('fin-scroll-y', y, -1, -1);
            });
        }
    }
    ...
    handleHScrollerChange(x: number) {
        ...
        if (updated) {
            ...
            setTimeout(() => {
                ...
                this.fireScrollEvent('fin-scroll-x', x, this.renderer.scrollAnchorColumnIndex, this.renderer.scrollAnchorColumnOffset);
     ...
     * @internal
     */
    getHScrollViewportStartColumnIndex() {
        if (this.properties.gridRightAligned) {
            return this.renderer.scrollOppositeAnchorColumnIndex;
        } else {
            return this.renderer.scrollAnchorColumnIndex;
        }
    }
    ...
    synchronizeScrollingBoundaries() {
        ...
        let lineGap = gridProps.gridLinesVWidth;
        // let columnsWidth = 0;
        ...
        lineGap = gridProps.gridLinesHWidth;
        let rowsHeight = 0;
        ...
        const horizontalScrollerContentStart = this.getFixedColumnsWidth();
        const scrollableColumnsViewportSize = bounds.width - horizontalScrollerContentStart;
        let horizontalScrollerContentFinish: number;
        let viewportStart: number;
        let viewportFinish: number;
        if (scrollableColumnsViewportSize <= 0) {
            horizontalScrollerContentFinish = horizontalScrollerContentStart - 1;
            viewportStart = horizontalScrollerContentStart;
            viewportFinish = horizontalScrollerContentStart - 1;
        } else {
            const horizontalScrollerContentSize = this.calculateScrollerContentSize(scrollableColumnsViewportSize, gridRightAligned);
            horizontalScrollerContentFinish = horizontalScrollerContentStart + horizontalScrollerContentSize - 1;

            if (gridRightAligned) {
                viewportFinish = this.renderer.calculateColumnScrollAnchorViewportFinish(horizontalScrollerContentFinish);
                viewportStart = viewportFinish - scrollableColumnsViewportSize + 1;
            } else {
                viewportStart = this.renderer.calculateColumnScrollAnchorViewportStart(horizontalScrollerContentStart);
                viewportFinish = viewportStart + scrollableColumnsViewportSize - 1;
            }
        }

        this.sbHScroller.contentRange = {
            start: horizontalScrollerContentStart,
            finish: horizontalScrollerContentFinish // hMax
        };
        this.sbHScroller.setViewport(viewportStart, viewportFinish);
    ...
    stashRowSelections() {
        if (this.properties.restoreRowSelections) {
            ...
            this.selectedDataRowIndexes = this.getSelectedRows().map(
                (selectedRowIndex) => {
                    return dataModel.getUnderlyingRowIndex(selectedRowIndex);
                }
            );
    ...
    unstashRowSelections() {
        ...
        if (dataRowIndexes) {
            ...
            for (let r = 0; selectedRowCount && r < rowCount; ++r) {
                const i = dataRowIndexes.indexOf(dataModel.getUnderlyingRowIndex(r));
                if (i >= 0) {
    ...
    /** @internal */
    private findOrCreateContainer(boundingRect: Hypegrid.BoundingRectStyleValues) {
        const div = document.getElementById('hypergrid');
        const used = div && !div.firstElementChild;

        if (!used) {
            const div = document.createElement('div');
            this.setStyles(div, boundingRect, Hypegrid.boundingRectStyleKeys);
    ...
    /** @internal */
    private setStyles<T extends Hypegrid.EdgeStyleValues | Hypegrid.BoundingRectStyleValues>(el: HTMLElement, style: T, keys: string[]) {
        if (style !== undefined) {
 ...
 *
 * For the dictionary of _instance_ plugins, see {@link Hypegrid#plugins|plugins} (defined in the {@link Hypegrid#intialize|Hypergrid constructor}).
 *
 ...
 * @desc Added in {@link Hypegrid constructor}; removed in {@link Hypegrid#terminate terminate()}.
 * Used in themes.js.
 * @type {Hypegrid[]}
 */
...
export namespace Hypegrid {
    ...
    export namespace Options {
        ...
        export interface Model {
            schema: (SchemaModel | SchemaModel.Constructor),
    ...
    export interface ColumnSortEventDetail {
        column: number;
        keys: string[];
    }
    ...
    export const grids = Array<Hypegrid>();

    export type ResizeSyntheticEventDetail = Canvas.ResizeSyntheticEvent;
    export type TouchSyntheticEventDetail = Canvas.TouchSyntheticEvent;
    export type KeyboardSyntheticEventDetail = Canvas.KeyboardSyntheticEvent;

    export interface EventMap {
        'fin-column-sort': CustomEvent<Hypegrid.ColumnSortEventDetail>;
        'fin-editor-keyup': CustomEvent<CellEditor.KeyEventDetail>;
        'fin-editor-keydown': CustomEvent<CellEditor.KeyEventDetail>;
        'fin-editor-keypress': CustomEvent<CellEditor.KeyEventDetail>;
        'fin-editor-data-change': CustomEvent<CellEditor.DataChangeEventDetail>;
        'fin-row-selection-changed': CustomEvent<SelectionDetail>;
        'fin-column-selection-changed': CustomEvent<SelectionDetail>;
        'fin-selection-changed': CustomEvent<SelectionDetail>;
        'fin-context-menu': CustomEvent<CellEvent>;
        'fin-mouseup': CustomEvent<CellEvent>;
        'fin-mousedown': CustomEvent<CellEvent>;
        'fin-mousemove': CustomEvent<CellEvent | undefined>;
        'fin-button-pressed': CustomEvent<CellEvent>;
        'fin-keydown': CustomEvent<KeyboardSyntheticEventDetail>;
        'fin-keyup': CustomEvent<KeyboardSyntheticEventDetail>;
        'fin-filter-applied': CustomEvent<undefined>;
        'fin-cell-enter': CustomEvent<CellEvent>;
        'fin-cell-exit': CustomEvent<CellEvent>;
        'fin-click': CustomEvent<CellEvent>;
        'fin-double-click': CustomEvent<CellEvent>;
        'fin-grid-rendered': CustomEvent<Hypegrid.GridEventDetail>;
        'fin-tick': CustomEvent<Hypegrid.GridEventDetail>;
        'fin-grid-resized': CustomEvent<ResizeSyntheticEventDetail>;
        'fin-touchstart': CustomEvent<TouchSyntheticEventDetail>;
        'fin-touchmove': CustomEvent<TouchSyntheticEventDetail>;
        'fin-touchend': CustomEvent<TouchSyntheticEventDetail>;
        'fin-scroll-x': CustomEvent<Hypegrid.ScrollEventDetail>;
        'fin-scroll-y': CustomEvent<Hypegrid.ScrollEventDetail>;
        'fin-request-cell-edit': CustomEvent<CellEditor.RequestCellEditDetail>;
        'fin-before-cell-edit': CustomEvent<CellEditor.DataChangeEventDetail>;
        'fin-after-cell-edit': CustomEvent<CellEditor.DataChangeEventDetail>;
        'fin-column-changed-event': CustomEvent<undefined>;
        'fin-hypergrid-columns-created': CustomEvent<undefined>;
    }
