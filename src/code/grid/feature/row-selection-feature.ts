...
export class RowSelectionFeature extends Feature {

    readonly typeName = RowSelectionFeature.typeName;
    ...
    override handleMouseDown(event: MouseCellEvent) {
        ...
        if (!rowSelectable || !event.isRowFixed) {
    ...
    override handleMouseDrag(event: MouseCellEvent) {
        ...
        if (
            this._dragArmed &&
            grid.properties.mouseRowSelection &&
            ...
            this.replaceLastSelectionAreaRowsFromExtendSelectOrigin(event.gridCell);
        } else {
    ...
    override handleKeyDown(eventDetail: EventDetail.Keyboard) {
        ...
        if (lastSelectionArea !== undefined) {
            ...
            if (lastSelectionType === SelectionArea.Type.Row) {
                const handler = this[('handle' + eventDetail.primitiveEvent.key) as keyof RowSelectionFeature] as ((this: void, detail: EventDetail.Keyboard) => void);
     ...
     */
    replaceLastSelectionAreaRowsFromExtendSelectOrigin(lastCellPoint: Point) {
        const extendSelectOrigin = this._extendSelectOrigin;
        if (extendSelectOrigin === undefined) {
    ...
    private checkDragScroll(mousePoint: Point) {
        ...
        if (
            ...
            scrollableBounds.containsPoint(mousePoint)
        ) {
        ...
        if (scrollBehavior.isScrollingActive()) {
            ...
            if (currentDrag === undefined) {
                ...
                if (currentDrag.y < b.topLeft.y) {
                    ...
                    if (currentDrag.y > b.topLeft.y + b.extent.y) {
                        yOffset = 1;
                    }
                }

                if (yOffset !== undefined) {
                    if (this._lastDragRow >= this.subgridsManager.calculateHeaderPlusFixedRowCount()) {
                        this._lastDragRow += yOffset;
                    }
                    scrollBehavior.scrollVBy(yOffset);
                }

                this.replaceLastSelectionAreaRowsFromExtendSelectOrigin(currentDrag.x, this._lastDragRow); // update the selection
    ...
    handleRIGHT() {
        ...
        if (mouseDown === undefined || extent === undefined) {
            ...
            let newX = grid.viewport.firstNonFixedColumnIndex;
...
export namespace RowSelectionFeature {
