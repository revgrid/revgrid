...
import { UnreachableCaseError } from '../lib/hypegrid-error';
...
export class CanvasRenderingContext2DEx {
    ...
    getTextWidthTruncated(this: CanvasRenderingContext2DEx,
        text: string,
        width: number,
        truncateType: GridProperties.TextTruncateType | undefined,
        abort: boolean,
        ...
        // width += truncateType === GridProperties.TextTruncateType.BeforeLastPartiallyVisibleCharacter ? 2 : -1; // fudge for inequality
        let sum = 0;
        if (truncateFromEnd) {
            ...
            for (let i = textLength - 1; i >= 0; --i) {
                ...
                if (truncating && sum > width && truncString === undefined) {
                    switch (truncateType) {
                        case GridProperties.TextTruncateType.WithEllipsis: { // truncate sufficient characters to fit ellipsis if possible
                        ...
                        case GridProperties.TextTruncateType.BeforeLastPartiallyVisibleCharacter: { // truncate *before* last partially visible character
                            truncString = text.substr(i + 1);
                            break;
                        }
                        case GridProperties.TextTruncateType.AfterLastPartiallyVisibleCharacter: { // truncate *after* partially visible character
