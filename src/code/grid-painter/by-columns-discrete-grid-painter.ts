...
export class ByColumnsDiscreteGridPainter extends GridPainter {
    ...
    paint(gc: CanvasRenderingContext2DEx) {
        ...
        this.visibleColumns.forEach((vc, c) => {
            let renderCell = pool[p]; // first cell in column c
            vc = renderCell.visibleColumn;

            prefillColor = renderCell.column.properties.backgroundColor;
            ...
            for (let r = 0; r < R; r++, p++) {
                renderCell = pool[p]; // next cell down the column (redundant for first cell in column)

                const config = renderCell.subgrid.getCellPaintConfig(renderCell);

                try {
                    preferredWidth = Math.max(preferredWidth, this.paintCell(gc, renderCell, config, prefillColor));
                } catch (e) {
                    this.renderer.renderErrorCell(this.grid, e, gc, vc, renderCell.visibleRow);
                }
            }

            gc.clipRestore(columnClip);

            renderCell.column.properties.preferredWidth = Math.ceil(preferredWidth);
        });
