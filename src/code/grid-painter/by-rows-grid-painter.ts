    ...
    paint(gc: CanvasRenderingContext2DEx) {
        ...
        for (let p = 0, r = 0; r < R; r++) {
            ...
            visibleColumns.forEach((vc, c) => {  // eslint-disable-line no-loop-func
                p++;
                const renderCell = pool[p]; // next cell across the row (redundant for first cell in row)
                ...
                try {
                    preferredWidth[c] = Math.max(preferredWidth[c], this.paintCell(gc, renderCell, config, prefillColor));
                } catch (e) {
        ...
        this.visibleColumns.forEach(function(vc, c) {
