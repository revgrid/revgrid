...
export class CellSelection extends Feature {
    ...
    override handleKeyDown(grid: Hypegrid, event: Canvas.KeyboardSyntheticEvent) {
        ...
        const navKey = cellEvent && (
                GridProperties.mappedNavKey(cellEvent.columnProperties.gridProperties, detail.char, detail.ctrl) ||
                GridProperties.navKey(cellEvent.columnProperties.gridProperties, detail.char, detail.ctrl)
            );
    ...
    handleMouseDragCellSelection(grid: Hypegrid, gridCell: Point, keys: string[]) {
     ...
     */
    checkDragScroll(grid: Hypegrid, mouse: Point) {
     ...
     */
    extendSelection(grid: Hypegrid, gridCell: Point, keys: string[]) {
    ...
    handleDOWN(grid: Hypegrid, event: CellEvent) {
    ...
    handleUP(grid: Hypegrid, event: CellEvent) {
        //keep the browser viewport from auto scrolling on key event
        event.primitiveEvent.preventDefault();
     ...
     */
    moveShiftSelect(grid: Hypegrid, offsetX: number, offsetY: number) {
