...
function translate(grid: Hypegrid, x: number, y: number) {
...
export class ColumnMoving extends Feature {
     ...
     */
    override initializeOn(grid: Hypegrid) {
    ...
    initializeAnimationSupport(grid: Hypegrid) {
     ...
     */
    floatColumnTo(grid: Hypegrid, draggedToTheRight: boolean) {
        this.floatingNow = true;

        const visibleColumns = grid.renderer.visibleColumns;
        const scrollLeft = grid.getHScrollViewportStartColumnIndex();
     ...
     */
    doColumnMoveAnimation(grid: Hypegrid, floaterStartX: number, draggerStartX: number) {
        return () => {
            const d = floatColumn;
     ...
     */
    doFloaterAnimation(grid: Hypegrid) {
     ...
     */
    createFloatColumn(grid: Hypegrid, columnIndex: number) {

        const fixedColumnCount = grid.getFixedColumnCount();
        let scrollLeft = grid.getHScrollViewportStartColumnIndex();
     ...
     * @param columnIndex - the index of the column that will be floating
     */
    createDragColumn(grid: Hypegrid, x: number, columnIndex: number) {
     ...
     * @param x - the start position
     */
    dragColumn(grid: Hypegrid, x: number) {
     ...
     * @param x - the start position
     */
    checkAutoScrollToRight(grid: Hypegrid, x: number) {
    ...
    _checkAutoScrollToRight(grid: Hypegrid, x: number) {
        if (!this.columnDragAutoScrollingRight) {
            return;
        }
        const scrollLeft = grid.getHScrollViewportStartColumnIndex();
        if (!grid.dragging || scrollLeft > (grid.sbHScroller.contentRange.finish - 2)) {
            return;
        }
     ...
     * @param x - the start position
     */
    checkAutoScrollToLeft(grid: Hypegrid, x: number) {
    ...
    _checkAutoScrollToLeft(grid: Hypegrid, x: number) {
        if (!this.columnDragAutoScrollingLeft) {
            return;
        }
