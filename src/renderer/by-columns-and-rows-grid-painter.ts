...
export class ByColumnsAndRowsGridPainter extends GridPainter {
    ...
    paint(gc: CanvasRenderingContext2DEx) {
        ...
        // clipToGrid,
        const viewWidth = C ? this.visibleColumns[C - 1].right : 0;
        ...
        // For each column...
        let p = 0;
        this.visibleColumns.forEachWithNeg(
            (vc, c) => {
                ...
                for (let r = 0; r < R; r++, p++) {
                    // if (!pool[p].disabled) {
                        ...
                        try {
                            preferredWidth = Math.max(preferredWidth, this.paintCell(gc, pool[p], prefillColor));
                        } catch (e) {
                            this.renderer.renderErrorCell(grid, e, gc, vc, pool[p].visibleRow);
                        }
                    // }
                }
