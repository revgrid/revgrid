...
export class ByRowsGridPainter extends GridPainter {
    ...
    paint(gc: CanvasRenderingContext2DEx) {
        ...
        // columnClip,
            // clipToGrid,
        const viewWidth = C ? visibleColumns[C - 1].right : 0;
        ...
        for (let p = 0, r = 0; r < R; r++) {
            ...
            visibleColumns.forEachWithNeg((vc, c) => {  // eslint-disable-line no-loop-func
                p++;
                const cellEvent = pool[p]; // next cell across the row (redundant for first cell in row)
                ...
                try {
                    preferredWidth[c] = Math.max(preferredWidth[c], this.paintCell(gc, cellEvent, prefillColor));
                } catch (e) {
                    this.renderer.renderErrorCell(grid, e, gc, vc, visibleRows[r]);
                }
