...
import { ColumnPropertiesAccessor } from './column-properties-accessor';
...
export class Column {
    ...
    readonly schema: DataModel.ColumnSchema;
     ...
     */
    constructor(behavior: Behavior, columnSchema: DataModel.ColumnSchema, index: number) {
        if (index === undefined) {
            throw new HypergridError('Column index required.');
        }

        this.behavior = behavior;
        this.dataModel = behavior.mainDataModel;
        this.index = index;
        const name = columnSchema.name;
        this.name = name === undefined ? this.index.toString() : name;

        // this.properties = columnSchema; // see {@link Column#properties properties} setter
        this._properties = new ColumnPropertiesAccessor(behavior.grid.properties, this); // see {@link Column#properties properties} setter
        ...
        switch (index) {
            case this.behavior.treeColumnIndex:
                ...
                const icon = images[Object.create(this.properties.treeHeader, { isDataRow: { value: true } }).leftIcon];
            ...
            default:
                if (index < 0) {
                    throw new HypergridError('New column index ' + index + ' out of range.');
                }
        }
    }
     ...
     * @type {string}
     */
    set calculator(calculator: DataModel.ColumnSchema.Calculator) {
    ...
    computeColumnType() {
        const headerRowCount = this.behavior.getHeaderRowCount();
        const height = this.behavior.getRowCount();
     ...
     */
    getCellProperties(rowIndex: number, subgrid: Subgrid): DataModel.CellOwnProperties | null {
        return this.getCellOwnProperties(rowIndex, subgrid); // ?? this.properties;
    }
     ...
     * @memberOf Column#
     */
    setCellProperties(rowIndex: number, properties: DataModel.CellOwnProperties | undefined, subgrid: Subgrid): DataModel.CellOwnProperties | undefined {
        if (properties) {
            return Object.assign(this.newCellPropertiesObject(rowIndex, subgrid), properties);
        } else {
            return undefined;
        }
    }
     ...
     * @memberOf Column#
     */
    addCellProperties(rowIndex: number, properties: DataModel.CellOwnProperties | undefined, subgrid: Subgrid): DataModel.CellOwnProperties | undefined {
        if (properties) {
     ...
     * @memberOf Column#
     */
    getCellOwnProperties(rowIndex: number, subgrid: Subgrid): DataModel.CellOwnProperties | null {
        // const dataModel = subgrid !== undefined ? subgrid.dataModel : this.dataModel;
        const metadata = subgrid.getRowMetadata(rowIndex);
        const properties = metadata && metadata[this.name] as DataModel.CellOwnProperties;
     ...
     */
    getCellProperty(rowIndex: number, key: string | number, subgrid: Subgrid): DataModel.CellOwnProperty {
        const cellProperties = this.getCellProperties(rowIndex, subgrid);
        if (cellProperties !== undefined && cellProperties !== null) {
            return cellProperties[key];
        } else {
            return undefined; // Should this be column Property?
        }
    }
     ...
     * @returns Cell's own properties object, which will be created by this call if it did not already exist.
     */
    setCellProperty(rowIndex: number, key: string, value: unknown, subgrid: Subgrid): DataModel.CellOwnProperties {
    ...
    clearAllCellProperties() {
        this.behavior.subgrids.forEach((subgrid) => {
     ...
     */
    private resolveCalculator(calculator: DataModel.ColumnSchema.Calculator): DataModel.ColumnSchema.CalculateFunction | undefined {
        if (!calculator) {
            return undefined;
        }
        ...
        let matches: string[];
        let key: string;
        const calculators = this.behavior.grid.properties.calculators ?? (this.behavior.grid.properties.calculators = {});
    ...
    private toFunction(string: DataModel.ColumnSchema.CalculateFunction | string | undefined): DataModel.ColumnSchema.CalculateFunction | undefined {
        ...
        return Function.apply(null, args) as DataModel.ColumnSchema.CalculateFunction;
    }
     ...
     * @param rowIndex - Data row coordinate.
     */
    private getCellPropertiesObject(rowIndex: number, subgrid: Subgrid): DataModel.CellOwnProperties {
        return this.getCellOwnProperties(rowIndex, subgrid) ?? this.newCellPropertiesObject(rowIndex, subgrid);
    }

    /**
     * @param rowIndex - Data row coordinate.
     */
    private newCellPropertiesObject(rowIndex: number, subgrid: Subgrid): DataModel.CellOwnProperties {
