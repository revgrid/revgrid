...
export abstract class CellEditor {
    ...
    constructor(public readonly grid: Hypergrid, options?: CellEvent, template?: string) {
    ...
    keyup(e: KeyboardEvent) {
        ...
        const keyChar = grid.canvas.getKeyChar(e);
        ...
        if (HypergridProperties.mappedNavKey(cellProps, keyChar, e.ctrlKey)) {
            if (
                !specialKeyup &&
            ...
            if (stopped) {
                ...
                const finEvent = grid.canvas.newEvent(e, 'fin-editor-keydown', {
                    primitiveEvent: e,
                    grid: grid,
                    alt: e.altKey,
                    ctrl: e.ctrlKey,
...
export namespace CellEditor {
    export type Constructor = new (grid: Hypergrid, options?: CellEvent, template?: string) => CellEditor;
    ...
    export interface DataChangeEvent extends Event {
        oldValue: unknown,
        newValue: unknown
    }

    export interface RequestCellEdit extends Event {
        value: unknown;
    }
}

// function nullPattern() {}
function px(n: string) { return n + 'px'; }
