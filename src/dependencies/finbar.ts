...
export class FinBar {
    ...
    constructor(options?: FinBar.Options) {
        ...
        this._min = options.range?.min ?? 0;
        this._max = options.range?.max ?? 100;
        this.contentSize = this._max - this._min + 1;
     ...
     * @param scaled - The new thumb position in pixels and scaled relative to the containing {@link FinBar#bar|bar} element, i.e., a proportional number in the range `0`..`thumbMax`.
     */
    _setScroll(idx: number, scaled?: number) {
        ...
        if (this.testPanelItem && this.testPanelItem.index instanceof HTMLElement) {
            this.testPanelItem.index.innerHTML = Math.round(idx).toString();
        }

        // Call the callback
        if (this.onchange) {
            this.onchange(Math.round(idx));
        }
     ...
     */
    resize(incrementOrBarStyles?: number | Record<string, string>, barStyles?: Record<string, string>) {
        const bar = this.bar;
    ...
    private _setThumbSize() {
        ...
        const thumbSize = Math.max(20, barSize * this.containerSize / this.contentSize);
    ...
    remove() {
        ...
        if (this.deltaProp !== null) {
            (this.container || this.bar.parentElement).removeEventListener('wheel', this._bound.onwheel);
        }
    ...
    onmousemove(evt: MouseEvent) {
        ...
        const scaled = Math.min(this._thumbMax, Math.max(0, evt[this.oh.axis] - this.pinOffset));
        const idx = scaled / this._thumbMax * (this._max - this._min) + this._min;
...
function validRange(range: FinBar.Range) {
